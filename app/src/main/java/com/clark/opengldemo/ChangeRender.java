 ...
 * 2022/1/9 19:09
 */
class ChangeRender implements GLSurfaceView.Renderer {
    private static final String TAG = ChangeRender.class.getCanonicalName();
    ...
    // 顶点数组、颜色数据
    private FloatBuffer vertices = ByteBuffer.allocateDirect(512 * 3 * 3 *4).order(ByteOrder.nativeOrder()).asFloatBuffer();//FloatBuffer.wrap(new float[512 * 3 * 3]);
    private FloatBuffer colors = ByteBuffer.allocateDirect(512 * 3 * 4 *4).order(ByteOrder.nativeOrder()).asFloatBuffer();//FloatBuffer.wrap(new float[512 * 3 * 4]);
    ...
    public ChangeRender(Context context) {
        mContext = context;
    }
    ...
    void objload(String name, OBJECT k) {
        ...
        //Log.e(TAG, "objload: "+k.points.toString());
